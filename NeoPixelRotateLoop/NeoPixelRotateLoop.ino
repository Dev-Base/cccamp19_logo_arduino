...
const uint16_t TailLength = 105; // length of the tail, must be shorter than PixelCount
...
void LoopAnimUpdate(const AnimationParam& param)
{
    ...
    if (param.state == AnimationState_Completed)
    {
        ...
        for (uint16_t index = 1; index <= strip.PixelCount()/* && index <= TailLength*/; index++)
        {
          HslColor oldColor = strip.GetPixelColor(index);
          float newHue = oldColor.H + 0.01;
          if(newHue > 1.0f) newHue = 0.0f;
          HslColor newColor = HslColor(newHue, oldColor.S, oldColor.L);
...
void DrawTailPixels()
{
    ...
    for (uint16_t index = 1; index <= strip.PixelCount()/* && index <= TailLength*/; index++)
    {
        ...
        if (lightness < 0.3) lightness = 0.3f;
